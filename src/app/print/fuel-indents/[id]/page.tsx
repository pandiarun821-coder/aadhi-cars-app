...
export default async function PrintFuelIndentPage({ params }: { params: Promise<{ id: string }> | { id: string } }) {
    ...
    <div className="min-h-screen bg-white text-black p-8 font-sans print:p-0 print:m-0 max-w-4xl mx-auto">
      ...
      <div className="border-2 border-black p-8 max-w-3xl mx-auto print:border-none print:p-4 print:max-w-none print:w-full">
        ...
        <div className="grid grid-cols-2 gap-y-6 gap-x-12 mb-12 text-lg">
          ...
          <div className="flex justify-between border-b border-gray-300 pb-1">
            <span className="font-semibold">Previous KM:</span>
            <span>{indent.previousKm || "-"}</span>
          </div>
          <div className="flex justify-between border-b border-gray-300 pb-1">
            <span className="font-semibold">Current KM:</span>
            <span>{indent.km}</span>
          </div>
          <div className="flex justify-between border-b border-gray-300 pb-1">
          ...
          <div className="flex justify-between border-b border-gray-300 pb-1">
            ...
            <span>{indent.location || "-"}</span>
          </div>
          <div className="flex justify-between border-b border-gray-300 pb-1">
            <span className="font-semibold">Petrol Agent:</span>
            <span>{indent.petrolAgent || "-"}</span>
          </div>
          <div className="flex justify-between border-b border-gray-300 pb-1 col-span-2">
            <span className="font-semibold">Remarks / Need:</span>
            <span className="text-right">{indent.remarks || "-"}</span>
          </div>
        </div>
